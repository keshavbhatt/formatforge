...
//--- MpvRenderer
void * MpvRenderer::get_proc_address(void * ctx,
                                    const char * name) {
    (void) ctx;
...
MpvObject::MpvObject(QQuickItem * parent): QQuickFramebufferObject(parent), m_enableAudio(true), m_useHwdec(false),
    ...
    WATCH_PROP_DOUBLE("container-fps-override")
...
void MpvObject::setProperty(const QString & name,
...
void MpvObject::setOption(const QString & name,
                          const QVariant & value) {
    ...
    case MPV_EVENT_PROPERTY_CHANGE: {
        ...
        if (prop -> format == MPV_FORMAT_NONE) {
            if HANDLE_PROP_NONE("vid", vid)
                    ...
                    else if HANDLE_PROP_NONE(
                        "sid", sid)
                    else if HANDLE_PROP_NONE("track-list/count",
                                              trackListCount)
            ...
            if (strcmp(prop -> name, "time-pos") == 0) {
                ...
                emit durationChanged(time);
                    ...
                    else if HANDLE_PROP_DOUBLE("display-fps",
                                                displayFps)
                    else if HANDLE_PROP_DOUBLE("estimated-display-fps",
                                                estimatedDisplayFps)
                    else if HANDLE_PROP_DOUBLE("estimated-vf-fps", estimatedVfFps)
                    else if HANDLE_PROP_DOUBLE("container-fps-override", fps)
                    else if HANDLE_PROP_DOUBLE("speed", speed)
                    else if HANDLE_PROP_DOUBLE("video-bitrate",
                                                videoBitrate)
                    else if HANDLE_PROP_DOUBLE("video-params/aspect",
                                                videoParamsAspect)
                    else if HANDLE_PROP_DOUBLE("video-out-params/aspect",
                                                videoOutParamsAspect)
                    else if HANDLE_PROP_DOUBLE("window-scale",
                                                windowScale)
                    else if HANDLE_PROP_DOUBLE("current-window-scale",
                                                currentWindowScale)

        } else if (prop -> format == MPV_FORMAT_FLAG) {
            if HANDLE_PROP_BOOL("idle-active", idleActive)
                else if HANDLE_PROP_BOOL("mute", muted)
                    else if HANDLE_PROP_BOOL(
                        "pause",
                        paused)
                    else if HANDLE_PROP_BOOL("paused-for-cache",
                                              pausedForCache)
                    else if HANDLE_PROP_BOOL("seekable",
                                              seekable)
