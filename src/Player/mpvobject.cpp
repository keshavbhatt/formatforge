...
MpvRenderer::MpvRenderer(const MpvObject *obj) : obj(obj), mpv_gl(nullptr) {
  ...
  mpv_render_context_set_update_callback(mpv_gl, MpvObject::on_update,
                                         (void *)obj);
}

MpvRenderer::~MpvRenderer() {
  qDebug() << "MpvRenderer destructor called";
  if (mpv_gl) {
...
MpvObject::MpvObject(QQuickItem *parent)
    : QQuickFramebufferObject(parent), m_enableAudio(true), m_useHwdec(false),
      m_duration(0), m_position(0), m_isPlaying(false) {
  ...
  mpv::qt::set_option_variant(mpv, "audio-client-name",
                              QApplication::applicationName().toUtf8().data());
  ...
  connect(this, &MpvObject::idleActiveChanged, this, &MpvObject::updateState);
  connect(this, &MpvObject::pausedChanged, this, &MpvObject::updateState);
...
void MpvObject::logPropChange(mpv_event_property *prop) {
  ...
  case MPV_FORMAT_NODE_MAP:
    ...
    break;
  default:
    qDebug() << objectName() << "prop(format=" << prop->format << ")"
             << prop->name;
    break;
  }
}
  ...
  case MPV_EVENT_PROPERTY_CHANGE: {
    ...
    if (prop->format == MPV_FORMAT_NONE) {
      if HANDLE_PROP_NONE ("vid", vid)
        else if HANDLE_PROP_NONE ("aid", aid) else if HANDLE_PROP_NONE (
            "sid", sid) else if HANDLE_PROP_NONE ("track-list/count",
                                                  trackListCount)
      ...
      if (strcmp(prop->name, "time-pos") == 0) {
        ...
        Q_EMIT durationChanged(time);
            ...
            "avsync", avsync) else if HANDLE_PROP_DOUBLE ("container-fps", containerFps) else if HANDLE_PROP_DOUBLE ("demuxer-cache-duration",
                                                                                                                     demuxerCacheDuration) else if HANDLE_PROP_DOUBLE ("display-fps",
                                                                                                                                                                       displayFps) else if HANDLE_PROP_DOUBLE ("estimated-display-fps", estimatedDisplayFps) else if HANDLE_PROP_DOUBLE ("estimated-vf-fps", estimatedVfFps) else if HANDLE_PROP_DOUBLE ("fps",
                                                                                                                                                                                                                                                                                                                                                         fps) else if HANDLE_PROP_DOUBLE ("speed", speed) else if HANDLE_PROP_DOUBLE ("video-bitrate",
                                                                                                                                                                                                                                                                                                                                                                                                                                      videoBitrate) else if HANDLE_PROP_DOUBLE ("video-params/aspect",
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                videoParamsAspect) else if HANDLE_PROP_DOUBLE ("video-out-params/aspect",
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               videoOutParamsAspect) else if HANDLE_PROP_DOUBLE ("window-scale", windowScale) else if HANDLE_PROP_DOUBLE ("current-window-scale",
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          currentWindowScale)

    } else if (prop->format == MPV_FORMAT_FLAG) {
      if HANDLE_PROP_BOOL ("idle-active", idleActive)
        else if HANDLE_PROP_BOOL ("mute", muted) else if HANDLE_PROP_BOOL (
            "pause",
            paused) else if HANDLE_PROP_BOOL ("paused-for-cache",
                                              pausedForCache) else if HANDLE_PROP_BOOL ("seekable",
                                                                                        seekable)

    } else if (prop->format == MPV_FORMAT_STRING) {
      if HANDLE_PROP_STRING ("audio-codec", audioCodec)
        else if HANDLE_PROP_STRING (
            "audio-codec-name",
            audioCodecName) else if HANDLE_PROP_STRING ("audio-params/format", audioParamsFormat) else if HANDLE_PROP_STRING ("filename",
                                                                                                                              filename) else if HANDLE_PROP_STRING ("file-format", fileFormat) else if HANDLE_PROP_STRING ("file-size",
                                                                                                                                                                                                                           fileSize) else if HANDLE_PROP_STRING ("audio-format", audioFormat) else if HANDLE_PROP_STRING ("hwdec", hwdec) else if HANDLE_PROP_STRING ("hwdec-current",
                                                                                                                                                                                                                                                                                                                                                                      hwdecCurrent) else if HANDLE_PROP_STRING ("hwdec-interop", hwdecInterop) else if HANDLE_PROP_STRING ("media-title", mediaTitle) else if HANDLE_PROP_STRING ("path",
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  path) else if HANDLE_PROP_STRING ("video-codec", videoCodec) else if HANDLE_PROP_STRING ("video-format",
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           videoFormat) else if HANDLE_PROP_STRING ("video-params/pixelformat", videoParamsPixelformat) else if HANDLE_PROP_STRING ("video-out-params/pixelformat",
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    videoOutParamsPixelformat) else if HANDLE_PROP_STRING ("ytdl-format",
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           ytdlFormat)

    } else if (prop->format == MPV_FORMAT_INT64) {
      if HANDLE_PROP_INT ("chapter", chapter)
        else if HANDLE_PROP_INT ("chapter-list/count", chapterListCount) else if HANDLE_PROP_INT (
            "decoder-frame-drop-count",
            decoderFrameDropCount) else if HANDLE_PROP_INT ("dwidth", dwidth) else if HANDLE_PROP_INT ("dheight",
                                                                                                       dheight) else if HANDLE_PROP_INT ("estimated-frame-count", estimatedFrameCount) else if HANDLE_PROP_INT ("estimated-frame-number", estimatedFrameNumber) else if HANDLE_PROP_INT ("frame-drop-count", frameDropCount) else if HANDLE_PROP_INT ("playlist-pos", playlistPos) else if HANDLE_PROP_INT ("playlist/count", playlistCount) else if HANDLE_PROP_INT ("vo-delayed-frame-count",
                                                                                                                                                                                                                                                                                                                                                                                                                                                                      voDelayedFrameCount) else if HANDLE_PROP_INT ("volume",
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    volume) else if HANDLE_PROP_INT ("vid", vid) else if HANDLE_PROP_INT ("aid", aid) else if HANDLE_PROP_INT ("sid", sid) else if HANDLE_PROP_INT ("audio-params/channel-count",
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    audioParamsChannelCount) else if HANDLE_PROP_INT ("audio-params/samplerate", audioParamsSampleRate) else if HANDLE_PROP_INT ("track-list/count",
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 trackListCount) else if HANDLE_PROP_INT ("contrast", contrast) else if HANDLE_PROP_INT ("brightness", brightness) else if HANDLE_PROP_INT ("gamma",
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            gamma) else if HANDLE_PROP_INT ("saturation",
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            saturation) else if HANDLE_PROP_INT ("sub-margin-y",
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 subMarginY)
...
void MpvObject::stepBackward() { command(QVariantList() << "frame-back-step"); }

void MpvObject::stepForward() { command(QVariantList() << "frame-step"); }
