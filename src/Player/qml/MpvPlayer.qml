...
Item {
    ...
    MpvObject {
        ...
        function contrastUp() {
            equalizerPropUp('contrast', 'Contrast')
        }
        function contrastDown() {
            equalizerPropDown('contrast', 'Contrast')
        }
        function gammaUp() {
            equalizerPropUp('gamma', 'Gamma')
        }
        function gammaDown() {
            equalizerPropDown('gamma', 'Gamma')
        }
        function brightnessUp() {
            equalizerPropUp('brightness', 'Brightness')
        }
        function brightnessDown() {
            equalizerPropDown('brightness', 'Brightness')
        }
        function saturationUp() {
            equalizerPropUp('saturation', 'Saturation')
        }
        function saturationDown() {
            equalizerPropDown('saturation', 'Saturation')
        }
        ...
        readonly property string timeLeftStr: formatShortTime(
                                                  duration - position)
        ...
        function formatTrackLabel(track) {
            var label = (track.lang ? " [" + track.lang + "]" : "")
        ...
        function nextVideoTrack() {
            ...
            if (mpvObject.vid > 0) {
                ...
                trackMsg = "" + mpvObject.vid + "/" + mpvObject.numVideoTracks
        ...
        function nextAudioTrack() {
            ...
            if (mpvObject.aid > 0) {
                ...
                trackMsg = "" + mpvObject.aid + "/" + mpvObject.numAudioTracks
                        + " " + formatTrackLabel(track)
            } else {
                trackMsg = "none"
            }
        ...
        function nextSubTrack() {
            ...
            if (mpvObject.sid > 0) {
                ...
                trackMsg = "" + mpvObject.sid + "/" + mpvObject.numSubTracks
                        + " " + formatTrackLabel(track)
            } else {
                trackMsg = "none"
            }
    ...
    MouseArea {
        ...
        onPositionChanged: {
            ...
            if (mouse.buttons & Qt.LeftButton == Qt.LeftButton) {
                ...
                var delta = Qt.point(globalCurPos.x - clickedMousePos.x,
                                     globalCurPos.y - clickedMousePos.y)
                var dist = Math.abs(delta.x) + Math.abs(
                            delta.y) // manhattanLength()
                ...
                var delta1 = Qt.point(curPos.x - cursorHiddenPos.x,
                                      curPos.y - cursorHiddenPos.y)
                var dist1 = Math.abs(delta1.x) + Math.abs(
                            delta1.y) // manhattanLength()
                var moveThreshold = 30
                if (dist1 >= moveThreshold) {
                    videoMouseArea.cursorShape = Qt.ArrowCursor
        ...
        Timer {
            ...
            onTriggered: {
                ...
                videoMouseArea.cursorHiddenPos = Qt.point(
                            videoMouseArea.mouseX, videoMouseArea.mouseY)
            }
        }
        ...
        onWheel: {
            ...
            for (; wheelDelta >= 120; wheelDelta -= 120)
                wheelUpTick()
            for (; wheelDelta <= -120; wheelDelta += 120)
                wheelDownTick()
        }
    ...
    function isSubExt(url) {
        ...
        var subExtList = ["utf", "utf8", "utf-8", "idx", "sub", "srt", "rt", "ssa", "ass", "mks", "vtt", "sup", "scc", "smi", "lrc", "pgs"]
    ...
    Item {
        id: overlayControls
        anchors.fill: parent
        readonly property bool showOverlay: headerBar.isHovered
                                            || controlBar.containsMouse
                                            || sidebar.containsMouse
                                            || hideCursorTimeout.running
        ...
        Behavior on opacity {
            NumberAnimation {
                duration: overlayControls.showOverlay ? 400 : 0
            }
        }
    ...
    Loader {
        ...
        Connections {
            ...
            function onLogMessage(prefix, level, text) {
                logModel.append({
                                    "prefix": prefix,
                                    "level": level,
                                    "text": text.trim()
    ...
    function isAutoplayingFolder() {
        return mpvObject.playlistCount === 1 && folderModel.count >= 2
                && config.autoplayNextFile && mpvPlayer.shouldAutoplay
    }
    function playPrevFileInFolder() {
        if (isAutoplayingFolder()) {
            ...
            if (currentFileIndex >= 0 && currentFileIndex > 0) {
                var prevFilePath = folderModel.get(currentFileIndex - 1,
                                                   'filePath')
    ...
    function playNextFileInFolder() {
        if (isAutoplayingFolder()) {
            ...
            if (currentFileIndex >= 0
                    && currentFileIndex < folderModel.count - 1) {
                var nextFilePath = folderModel.get(currentFileIndex + 1,
                                                   'filePath')
                mpvPlayer.shouldAutoplay = false
